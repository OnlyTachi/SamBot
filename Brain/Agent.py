...
class CerebroIA(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        if MEMORY_AVAILABLE:
            ...
            self.identity = data_manager.get_identity() if hasattr(data_manager, 'get_identity') else {"name": "SamBot"}
        else:
        ...
        if MEMORY_AVAILABLE:
            ...
            self.active_channels = raw_channels if isinstance(raw_channels, dict) else {}
    ...
    def _inicializar_ferramentas(self):
        ...
        for key, cls in tool_map.items():
            try:
                ...
                try: self.tools[key] = cls(self.bot)
                except: self.tools[key] = cls()
    ...
    async def _consultar_memoria_longa(self, user_id: str, query: str) -> str:
        ...
        try:
            fatos = vector_store.query_relevant("fatos_usuario", f"user:{user_id} {query}", n_results=2)
    ...
    async def _aprender_fatos(self, message: discord.Message, clean_text: str):
        """Identifica e salva informações pessoais no VectorStore."""
        if not MEMORY_AVAILABLE: return
        gatilhos = ["meu nome é", "eu gosto de", "eu moro em", "sou ", "meu aniversário", "trabalho com"]
        if any(g in clean_text.lower() for g in gatilhos):
            try:
                doc_id = f"fact_{message.author.id}_{int(time.time())}"
                vector_store.add_memory(
                    "fatos_usuario",
                    f"O utilizador {message.author.name} afirmou: {clean_text}",
                    {"user_id": str(message.author.id), "timestamp": str(datetime.now())},
                    doc_id
                )
            except Exception as e:
                logger.error(f"❌ Erro ao gravar fato: {e}")
    ...
    async def _rotear_ferramentas(self, content: str, provider) -> str:
        ...
        router_prompt = (
            "Analise a solicitação e responda APENAS um JSON puro.\n"
            "Ferramentas: 'weather', 'game_search', 'image_search', 'web_search', 'music_recommend', 'none'.\n"
            f"Usuário: {content}"
        )
        ...
        try:
            decisao_raw = await provider.generate_response(
                ...
                system_prompt="Responda apenas JSON. Ex: {\"tool\": \"weather\", \"args\": \"Lisboa\"}"
            )
            ...
            logger.info(f"🛠️ Tool: {tool_name} | Args: {args}")
            ...
            if tool_name == "image_search":
                ...
                # Assume-se que as outras são assíncronas
                method_name = "get_weather" if tool_name == "weather" else \
                              "recommend_music" if tool_name == "music_recommend" else \
                              "search_game" if tool_name == "game_search" else "buscar_na_cascata"
                res = await getattr(tool_obj, method_name)(str(args))
            
            return f"\n[RESULTADO DA FERRAMENTA {tool_name.upper()}]:\n{res}\n"
                
        except Exception as e:
            logger.warning(f"⚠️ Falha no roteamento: {e}")
        return ""
    ...
    def _is_command(self, message: discord.Message, prefixes) -> bool:
        ...
        # 1. Verifica prefixos oficiais do bot
        if isinstance(prefixes, str):
            if content.startswith(prefixes): return True
        elif isinstance(prefixes, list):
            if any(content.startswith(p) for p in prefixes): return True
            
        # 2. Verificação de segurança (Hardcoded) para casos comuns de outros bots
        common_prefixes = ["!", "/", ".", "?", "+", "-", "$", "%", "&", "*"]
        if any(content.startswith(p) for p in common_prefixes) and len(content) > 1:
            # Verifica se a segunda letra não é um espaço (ex: "!hug" vs "! oi")
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_mentioned or is_dm or is_reply or chance_aleatoria:
            async with message.channel.typing():
                ...
                # Normalização via DataManager/Limpeza se disponível
    ...
    async def processar_cognicao(self, message: discord.Message, clean_text: str, persona_name: str):
        ...
        try:
            ...
            # 1. Aprendizado e Reflexão
            await self._aprender_fatos(message, clean_text)
            if MEMORY_AVAILABLE and self.autoconhecimento.is_self_inquiry(clean_text):
                resp = await provider.generate_response(clean_text, self.autoconhecimento.get_identity_prompt())
            ...
            historico_chat = ""
            if MEMORY_AVAILABLE:
                historico_chat = await self.historico.get_formatted_history(message, self.bot.user)
            ...
            resposta = await provider.generate_response(
                user_prompt=f"{user_name}: {clean_text}", 
                system_prompt=full_system
            )

            # 5. Emoção (Se não houver emoji na resposta)
            if MEMORY_AVAILABLE and not any(e in resposta for e in ["😀", "🤔", "✨", "💜"]):
                resposta += f" {self.expressoes.get_reaction(clean_text)}"

            logger.info(f"🗣️ Resposta para {user_name} gerada em {time.time()-start_time:.2f}s")
    ...
    async def _enviar_resposta(self, message: discord.Message, texto: str):
        """Garante o envio respeitando limites do Discord."""
        if not texto: return
